...
export interface ImageAnnotatorProps {
  ...
  readOnly?: boolean;
  className?: string;
  // External state props for integration with parent components
...
export function ImageAnnotator({
  ...
  const [internalColor, setInternalColor] = useState("#FF0000");
  const [internalStrokeWidth, setInternalStrokeWidth] = useState(2);

  const currentTool = externalTool ?? internalTool;
  const currentColor = externalColor ?? internalColor;
  ...
  return (
    <div className={`flex flex-col h-full ${className}`}>
      {/* Toolbar - show only when not in readOnly mode and showToolbar is true and external state is not provided */}
      {!readOnly && showToolbar && !externalTool && (
        <AnnotationToolbar
          currentTool={currentTool}
          onToolChange={setInternalTool}
          currentColor={currentColor}
          onColorChange={setInternalColor}
          strokeWidth={strokeWidth}
          onStrokeWidthChange={setInternalStrokeWidth}
          onUndo={handleUndo}
          onClear={handleClear}
