...
import { Modal, Form, Input, DatePicker, Button as AntButton, Space, Select, App, Tabs } from "antd";
...
export function CreateCarePlanModal({
  ...
  // Convert dayjs objects to date strings (Ant Design DatePicker returns dayjs objects)
  ...
  const loadPlanDetails = async () => {
    ...
    try {
      ...
      form.setFieldsValue({
        ...
        // Note: date_range not set here as Ant Design DatePicker requires dayjs objects
        // User will need to reselect dates when editing
    ...
    try {
      ...
      if (editingPlan) {
        ...
        const planData: CreateCarePlanRequest = {
          ...
          plan_name: values.plan_name,
          plan_description: values.plan_description,
          start_date: formatDate(dateRange?.[0]) || new Date().toISOString().split("T")[0],
          end_date: formatDate(dateRange?.[1]),
          medications,
          tasks,
          precautions,
