 ...
 * Frontend API client for patient records
 */
...
/**
 * Create a patient todo
 */
export async function createPatientTodo(
  todoData: CreateTodoRequest
): Promise<ApiSuccessResponse> {
  try {
    const response = await fetch(`${API_BASE_URL}/patient/todo/create`, {
      method: "POST",
      headers: {
        ...getAuthHeaders(),
        "Content-Type": "application/json",
...
export async function getPatientTodos(
  ...
  try {
    const url = status
      ? `${API_BASE_URL}/patient/${patientId}/todos?status=${status}`
      : `${API_BASE_URL}/patient/${patientId}/todos`;
...
export async function updateTodoStatus(
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/patient/todo/${todoId}/status`, {
      method: "PUT",
      headers: {
        ...getAuthHeaders(),
        "Content-Type": "application/json",
