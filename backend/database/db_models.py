...
class CaseInfo(TableBase):
    """
    Case library information table
    """
    __tablename__ = "case_info_t"
    __table_args__ = {"schema": SCHEMA}

    case_id = Column(Integer, Sequence("case_info_t_case_id_seq",
                        schema=SCHEMA), primary_key=True, nullable=False, doc="Case ID, primary key")
    case_no = Column(String(50), doc="Case number (e.g., #0234)")
    diagnosis = Column(String(500), doc="Primary diagnosis")
    patient_age = Column(Integer, doc="Patient age at the time of diagnosis")
    patient_gender = Column(String(10), doc="Patient gender: 男/女")
    chief_complaint = Column(Text, doc="Chief complaint")
    present_illness = Column(Text, doc="History of present illness")
    past_medical_history = Column(Text, doc="Past medical history")
    physical_examination = Column(Text, doc="Physical examination findings")
    imaging_findings = Column(Text, doc="Imaging examination findings (X-ray, CT, MRI, etc.)")
    treatment_plan = Column(Text, doc="Treatment plan and medications")
    prognosis = Column(Text, doc="Prognosis and follow-up outcomes")
    symptoms = Column(JSON, doc="Key symptoms (JSON array)")
    similarity_score = Column(Numeric(5, 2), doc="Similarity score (0-100) for AI matching")
    tenant_id = Column(String(100), doc="Tenant ID")


class CaseLaboratoryTest(TableBase):
    """
    Case laboratory test results table
    """
    __tablename__ = "case_laboratory_test_t"
    __table_args__ = {"schema": SCHEMA}

    test_id = Column(Integer, Sequence("case_laboratory_test_t_test_id_seq",
                        schema=SCHEMA), primary_key=True, nullable=False, doc="Test ID, primary key")
    case_id = Column(Integer, doc="Case ID, foreign key")
    test_name = Column(String(100), doc="Test name (e.g., RF, 抗CCP抗体, ESR, CRP)")
    test_value = Column(String(100), doc="Test value")
    test_unit = Column(String(50), doc="Unit (e.g., IU/mL, mm/h, mg/L)")
    normal_range = Column(String(100), doc="Normal range")
    is_abnormal = Column(Boolean, doc="Whether the result is abnormal")
    tenant_id = Column(String(100), doc="Tenant ID")
