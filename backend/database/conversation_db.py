...
def get_conversation_list(
    ...
    """
    with get_db_session() as session:
        # Build the query statement - now select all fields
        stmt = select(
            ConversationRecord.conversation_id,
            ConversationRecord.conversation_title,
            ConversationRecord.portal_type,
            ...
            ConversationRecord.archived_at,
        ...
        for record in records:
            ...
            # Convert archived_at to timestamp if present
            if conversation.get('archived_at'):
                archived_timestamp = int(conversation['archived_at'].timestamp() * 1000)
                conversation['archived_at'] = archived_timestamp
