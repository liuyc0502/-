...
async def get_patient_care_plans(
    ...
    """
    try:
        plans = list_care_plans_by_patient(medical_record_no, tenant_id, status)
...
async def update_care_plan_progress_tool(
    ...
    try:
        ...
        for item in completed_items:
            ...
            completion_data = {
                "plan_id": plan_id,
                "patient_id": None,  # Will be set from plan if needed
            ...
            # Get plan to get patient_id
            plan = get_care_plan_detail(plan_id, tenant_id)
            if plan:
                completion_data["patient_id"] = plan.get("patient_id")
