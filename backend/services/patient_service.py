...
async def create_annotation_service(annotation_data: dict, tenant_id: str, user_id: str) -> dict:
    """
    Create a new image annotation
    """
    try:
        result = patient_db.create_image_annotation(annotation_data, tenant_id, user_id)
        return {
            "success": True,
        ...
        raise AgentRunException(f"Failed to create annotation: {str(e)}")


async def get_annotations_service(image_id: int, tenant_id: str) -> List[dict]:
    """
    Get all annotations for an image
    """
    try:
        annotations = patient_db.get_image_annotations(image_id, tenant_id)
        return annotations
    except Exception as e:
        logger.error(f"Failed to get annotations: {str(e)}")
        raise AgentRunException(f"Failed to get annotations: {str(e)}")


async def update_annotation_service(annotation_id: int, update_data: dict, tenant_id: str, user_id: str) -> dict:
    """
    Update an annotation
    """
    try:
        success = patient_db.update_annotation(annotation_id, update_data, tenant_id, user_id)
        if not success:
        ...
        raise AgentRunException(f"Failed to update annotation: {str(e)}")


async def delete_annotation_service(annotation_id: int, tenant_id: str, user_id: str) -> dict:
    """
    Delete an annotation
    """
    try:
        success = patient_db.delete_annotation(annotation_id, tenant_id, user_id)
        if not success:
            raise ValueError("Annotation not found")
        ...
        raise AgentRunException(f"Failed to delete annotation: {str(e)}")
...
async def analyze_region_service(
    annotation_id: int,
    question: str,
    ...
    (This is a placeholder - in production, this would call the AI vision model)
    """
    try:
        # Get annotation details
        annotation = patient_db.get_annotation_by_id(annotation_id, tenant_id)
        if not annotation:
        ...
        analysis_result = f"""Based on analysis of {region_name} ({annotation.get('annotation_label', '')}):

Question: {question}

[Note: This is a placeholder response. In production, integrate with GPT-4V or Claude 3.5 Sonnet for actual image analysis]
...
"""
        ...
        result = patient_db.create_image_analysis(analysis_data, tenant_id, user_id)
        ...
        raise AgentRunException(f"Failed to analyze region: {str(e)}")