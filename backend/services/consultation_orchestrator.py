...
class ConsultationOrchestrator:
    ...
    def run_consultation(
        ...
        try:
            ...
            for round_num in range(1, max_rounds + 1):
                ...
                # Emit waiting for doctor (unless last round by max)
                self.observer.add_message(
    ...
    def _run_specialist_round(
        ...
        round_result.consensus_level = self._assess_consensus(round_result)
        return round_result
    ...
    @staticmethod
    def _assess_consensus(round_result: RoundResult) -> str:
        """Assess consensus level based on opinions."""
        opinions = list(round_result.opinions.values())
        if len(opinions) <= 1:
            return "full"

        # Check if anyone disagrees with anyone
        has_disagreement = any(o.disagrees_with for o in opinions)
        all_agree = all(
            len(o.agrees_with) >= len(opinions) - 1
            for o in opinions
        )

        if all_agree and not has_disagreement:
            return "full"
